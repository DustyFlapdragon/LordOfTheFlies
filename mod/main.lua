...
____exports.default = (function()
    ...
    function Config.prototype.____constructor(self)
        self.locustOfWar = true
        self.locustOfPestilence = true
        self.locustOfFamine = true
        self.locustOfDeath = true
        self.locustOfConquest = true
        self.appolyonsBestFriend = true
        self.theD6 = true
        self.smartFly = true
        self.psyFly = true
        self.lostFly = true
        ...
        self.angryFly = true
        self.botFly = true
        self.fruityPlum = true
        self.skatole = true
        self.haloOfFlies = true
        self.distantAdmiration = true
        self.foreverAlone = true
        self.bbf = true
        self.bigFan = true
        self.friendZone = true
...
local ITEMS = ____Config.ITEMS
local TRINKETS = ____Config.TRINKETS
local CATEGORY_NAME, INFO_PANEL, boolStatus, deleteOldConfig, addSubMenuItem, addInfoMenuItem
...
function addSubMenuItem(self, subMenuName, options)
    for ____, ____value in ipairs(options) do
        local configName
        configName = ____value[1]
        local array
        array = ____value[2]
        local itemID, optionType = table.unpack(array)
        local optionName = ""
        if subMenuName == "Items" then
        ...
        ModConfigMenu.AddSetting(
            ...
            subMenuName,
            {
                ...
                Display = function() return (optionName .. ":") .. boolStatus(nil, g.config[configName]) end,
...
function addInfoMenuItem(self)
    ModConfigMenu.AddText(
        CATEGORY_NAME,
        INFO_PANEL,
        function() return "Lord of the Flies" end
    )
    ModConfigMenu.AddText(
        CATEGORY_NAME,
        INFO_PANEL,
...
CATEGORY_NAME = "Lord of The Flies"
INFO_PANEL = "Info"
function ____exports.register(self)
    ...
    deleteOldConfig(nil)
    addInfoMenuItem(nil)
    addSubMenuItem(nil, "Items", ITEMS)
    addSubMenuItem(nil, "Trinkets", TRINKETS)
...
local saveData = require("saveData")
local LordOfTheFlies = RegisterMod("LordOfTheFlies", 1)
saveData:setMod(LordOfTheFlies)
saveData:load()
LordOfTheFlies:AddCallback(ModCallbacks.MC_POST_GAME_STARTED, postGameStarted.main)
LordOfTheFlies:AddCallback(ModCallbacks.MC_ENTITY_TAKE_DMG, playerTakeDmg.main, EntityType.ENTITY_PLAYER)
LordOfTheFlies:AddCallback(ModCallbacks.MC_POST_PLAYER_INIT, postPlayerInit.main)
